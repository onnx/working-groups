...
- [ ] Include factorizations of attributes and constraints and the sentence about general constraints in the guidelines and template  [project::[[SONNX]]] 

##### Update to preamble / lexicon etc.
- Introduce a "preamble" / "introduction" that will express some important genral information and disclaimers
...
- The specification will be written using a certain set of "basic" operator that will nt be defined abny further. This includes at least `+`, `-`, `*`, `/`, `exp`, `sin`, etc.  They have to be defined in the "preamble" or "introduction" to SNNX.
- [ ] Define the set of basic operators in the SONNX introduction [project::[[SONNX]]] 
...
- [ ] Check if infs ($\pm \infty$) belong to set the of real numbers [project::[[SONNX]]]  
...
- [ ] In the examples, use $\approx$ instead of $=$ when applicable [project::[[SONNX]]] 
- [ ] The jupyternotebook used to compute the examples must be provided [project::[[SONNX]]]   

##### Tests 
- We need a clear functional testing strategy. We canot only rely to the tests automatically generated by Hyptotheses. We may provide guidance on a "per operator class" basis. For instance, we may give specific guidance for operators using padding, etc.
- [ ] Elaborate a clear testing strategy [project::[[SONNX]]]  
- The main test oracle is ORT. However, in some cases (e.g., MaxPool) the behaviour of ORT may be different from what we would expect (exemple: effect of inf in inputs). In that case, if the behaviour is not a manisfest error (such as the case of "-4)...), we will add a notice explaining the discrependancy between our spec and the observable behaviour of ORT. 
-  Move the exsting tests in the test folder [project::[[SONNX]]]  

- [ ] Check the behaviour of MaxPool with ORT on a GPU executor. [project::[[SONNX]]]  
