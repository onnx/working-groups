...
to the SONNX informal specifiation that would implement integer arithmetic or
floating point arithmetic according to IEEE-754.

Towards that goal, SONNX provides:
- an analytical expression of the upper bound of the error considering the
  SONNX specification applying integer or IEEE-754 arithmetic and
  under a narrow set of acceptable hypotheses
- a tool to estimate numerically an upper bound of the error of a given implementation.
  
Notes:  
1. There may exist multiple SONNX algorithms for any operator. In SONNX, we use
...
3. The acceptable hypotheses provide concrete or symbolic ranges for the imput values
...
approximated computations and a SONNX operation $op_{\textit{ideal}}$
operating on data also coming from previous SONNX computations:
...
In order to simplify the calculus and preserve the readability of the error
estimation formulas, we apply conservative simplifications of the native
computer operations (e.g., IEEE-754). In some
...
The error specification comes with unit verification scenarios to verify an
...
In some cases, additional hypotheses about the CPU, the organisation of
the input data facilitate the verification. Hence, an implementation that
claims to comply with the accuracy specification must define the specification
hypotheses under which it has been verified.

The implementer should also define the verification method with the guarantees it brings.
...
* manual (or aided with tools) symbolic reasoning about the accuracy of
  the implementation,  
* static analysis verification with additional ranges hypotheses for the
...
* dynamic analysis synchronous stochastic verification - CPU or GPU,
  off-line for big components, on-line for small components,  
...
* dynamic analysis asynchronous concrete verification - CPU or GPU,
  could be on-line for big components.
...
implementation. Hence, any implementation verifying the SONNX informal specification
of the operator automatically verifies the error propagation formula.

The formula nevertheless explains how an input error is amplified by
the operator just from its functional description. It is very useful
for the error specification of a graph of operators since the numerical error
introduced by the first operator is then amplified by the next operators.

From the theoretical point of view, let us consider $op_{ideal}$ as
$\textbf{op}: \mathbb{R}^n \longrightarrow \mathbb{R}^m$ and an input vector
error $E = (e^i)$ for the vector argument $X = (x^i)$ with $i \in [0, n-1]$.

### Estimation of the propagated error

The **propagated error** of $\textbf{op} = (f^0, \ldots, f^{m-1})$
as SONNX operator is $\forall 0 \leq j < m$

$$f^j(x^0 + e^0, \ldots, x^{n-1} + e^{n-1}) - f^j(x^0, \ldots, x^{n-1})$$

Hence if $\textbf{op}$ is derivable two times, the formula $\forall 0 \leq j < m$

$$\sum_{0 \leq i < n} \frac{\delta f^j}{\delta x^i}
  (x^0, \ldots, x^{n-1})\times e^i + \mathcal{O}^j(E^2)
...
$$\mathcal{O}^j(E^2) = \left(\textbf{op}(X + E) - \textbf{op}(X)\right) -
  (\textbf{J}_{\textbf{op}}) (E)$$

where $\textbf{J}_{\textbf{op}}$ is the Jacobian matrix of the function $\textbf{op}$.
...
$$(\textbf{J}_{\textbf{op}})(E) + \left(\textbf{op}(X + E) - \textbf{op}(X) - (\textbf{J}_{\textbf{op}})(E)\right)$$
...
implementation result and the SONNX result when there is no error on the input.
...
the difference between the implementation result and the SONNX result, for
...
* The `real` field is a symbolic abstract domain for SONNX (infinitely precise)
  computations.
