...
def check_edges():
    ...
    with open("generated_data.json", "r", encoding="utf-8") as f:
        ...
        check_axis = check_individual_variables(
            "axis", data["axis"], -data["max_rank_input"], data["max_rank_input"])

        return all([check_rank_input, check_shape_size_input, check_x_type, check_axis])
...
def check_type(variable_name, variable_analysis, provider):
    ...
    Check input types
    """
    supported_types = set(flatten_types.get(provider).keys())
