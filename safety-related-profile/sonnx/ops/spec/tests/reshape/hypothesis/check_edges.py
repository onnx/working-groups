"""
This file checks edge cases in the generated data for the Reshape operation in ONNX.
"""
import json
import numpy as np
import ml_dtypes
...
def check_edges():
    ...
    with open("generated_data.json", "r", encoding="utf-8") as f:
        ...
        check_rank_input_x = check_individual_variables(
            "rank_input_tensor_x", data["rank_input_tensor_x"], 
            data["min_rank_input"], data["max_rank_input"])
        
        check_rank_input_s = check_individual_variables(
            "rank_input_tensor_s", data["rank_input_tensor_s"], 
            data["min_rank_input"], data["max_rank_input"])

        check_shape_size_input_x = check_individual_variables(
            "shape_input_tensor_x", 
            [size for shape in data["shape_input_tensor_x"] for size in shape], 
            data["min_dim_size_input"], data["max_dim_size_input"])
        
        check_shape_size_input_s = check_individual_variables(
            "shape_input_tensor_s", 
            [size for shape in data["shape_input_tensor_s"] for size in shape], 
            data["min_dim_size_input"], data["max_dim_size_input"])
        
        provider = data["ONNXRuntime_Provider"]

        check_input_type = check_type("input_type", data["input_type"], provider)

        return all([check_rank_input_x, check_rank_input_s, check_shape_size_input_x, check_shape_size_input_s, check_input_type])
...
def check_type(variable_name, variable_analysis, provider):
    ...
    Check input types
    """
    supported_types = set(reshape_types.get(provider).keys())
