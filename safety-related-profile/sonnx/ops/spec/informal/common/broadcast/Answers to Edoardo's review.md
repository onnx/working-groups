...
*Moreover, it depends on the the operator using broadcasting... For instance the Where operator makes broadcasting from one bool tensor and two other data types tensors. I think that "numerical type" is the little restrictive because it is also applicable for string. May be "the entries of the tensors Xm and Zm must have the same data type."*
