...
**[EM: I would say something prescriptive like "Any SONNX-compliant implementation of MatMul shall provide sound error propagation bounds in the form outlined above. In the remainder of the document, we show how to manually compute such bounds for a naive reference implementation of MatMul. More precise implementations might exist, but the derivation of their bounds is left to the implementor."]**
**[FV: Done]**
...
**[EM: as per my note on the general guidelines, this choice of keeping a single term for the whole matrix is quite strong. The over-approximated error will be extremely amplified as the SONNX graph grows large. This is a typical problem in abstract interpretation of neural networks: all state-of-the-art tools (e.g. $\alpha\beta$-CROWN, PyRAT) keep a full error tensor when propagating through MatMul and GEMM operators (at least when either A or B are constant). Even in that case, the error may grow unacceptably large: hence, all efforts to reduce over-approximation MUST be taken.]**
**[FV: we now provide two specification: one to check the implementation on a worst-case and one to infer the accuracy of a SONNX-graph in a modular approach]**
...
A_{\textit{err}}[i, k]\times B[k, j]\right) + \left(\sum_{k=0}^{n-1} A[i,
k]\times B_{\textit{err}}[k, j]\right) +\mathcal{O}(|A_{\textit{err}}|\times
|B_{\textit{err}}|)$ **[EM: some absolute values are missing here! Note however
how some of the terms will be smaller than $|A_{err}|$, $|B_{err}|$ and, even
more importantly, positive and negative terms have a chance to cancel out! Both
would yield a much tighter estimation of the propagated error.]**
...
This operator amplifies an initial error by absolute value of the coefficients
of the other matrix. **[EM: and we should try to avoid that.]** **[FV: Done]**
...
the terms of the sum and/or with more accurate local additions and
multiplications. **[EM: add prescriptive statement, e.g. "any such
implementation must be accompanied by a valid error bound to be
...
This formula is obtained with a proof by induction over $n$. **[EM: a few more
words on the proof rationale would be appreciated]** **[FV: Done, copy-paste from accuracy.md]**
