...
This operator amplifies an initial error by absolute value of the coefficients of the other matrix. **[EM: and we should try to avoid that.]**
...
the terms of the sum and/or with more accurate local additions and multiplications. **[EM: add prescriptive statement, e.g. "any such implementation must be accompanied by a valid error bound to be SONNX-compliant. The job of producing such error bounds is left to the implementor."]**
...
This formula is obtained with a proof by induction over $n$. **[EM: a few more words on the proof rationale would be appreciated]**
