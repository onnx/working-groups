...
> Use lowercases for all types
> For FP numbers, use ONNX standard: float16, float, double... 
...
> Use ONNX names ("input", "min", "max", "output")
> Proposal:
...
> At interface level we must use ONX names ("min", "max",...). But for the formulae, we should probably stick to mathematical symbols.
...
> /!\ See formatting of ">" and "<". Do as shown above.
> Remove "forall i" or introduce it at the beginning with a clear definition of the domain of i.
...
> Don't forget to rename the tensor according to the interface ("min",...)
...
> Add the following constraint (also for "max"):
> 
> The shape of tensor "min" must be empty. 
...
> Replace by a link to the (not yet existing) : "clip_na.md" at the same level as the informal spec. 
...
> Add a remark about the fact that the values used in all arguments are of the same type. (event though it is implicit).  
...
> replace the list of types by "int" which is defined in the interface, which gives:
> **input: int**
...
> For such modification, maybe use ChatGPT...
...
> Considering your example (onnxruntime), L must be substituted by -inf

  - If $M$ is $nan$, it is set to the highest representable value.

> Considering your example (onnxruntime), M must be substituted by +inf

- The clipping is then performed as:

> Move the previous items about NaNs before the spec "if L <= M ..."
