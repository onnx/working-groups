...
@st.composite
def valid_conv_args(draw, inputs_attributes=inputs_attributes, tensor_range=tensor_range):
    # A float strategy
    a_float = st.floats(min_value=tensor_range['min_value'], max_value=tensor_range['max_value'], 
    ...
    # Input X
    #---------------------------------------------------
    # dx0 is the first dimension of X, i.e., the dimension of the batch size
    dx0 = draw(st.integers(min_value=inputs_attributes['min_dx0'], 
                           max_value=inputs_attributes['max_dx0']))
    
    # dx1 is the second dimension of X, i.e., the number of channels
    dx1 = draw(st.integers(min_value=inputs_attributes['min_dx1'],
                            max_value=inputs_attributes['max_dx1']))
    
    # X [C1]

    x_spatial_axis = draw(st.lists(
                            st.integers(min_value=inputs_attributes['x_spatial_axis_min'], 
                                        max_value=inputs_attributes['x_spatial_axis_max']),
                            min_size=num_spatial_axes_x,
                            max_size=num_spatial_axes_x))
    # X [C4]
    x = draw(hnp.arrays(dtype=np.float32, 
                        shape=(dx0, dx1, *x_spatial_axis),
                        elements=a_float))
    ...
    #---------------------------------------------------    
    # Strides [C1]
    stride_value = st.integers(min_value=inputs_attributes['strides_min'], 
                            max_value=inputs_attributes['strides_max'])
    # Strides [C2] 
    ...
    # Groups [C2]
    # Groups [C3]
    all_valid_groups = [1, dx1]
    groups = draw(st.sampled_from(all_valid_groups))
    inputs_attributes['groups'].append(groups)
    

    #---------------------------------------------------
    # Attributes autopad, pads and dilations
    #---------------------------------------------------
    # Auto Pad [C1]
    all_valid_autopad = ["NOTSET", "SAME_UPPER", "SAME_LOWER", "VALID"] 
    auto_pad = draw(st.sampled_from(all_valid_autopad))
    
    # Auto Pad [C2]
    if auto_pad == "NOTSET" or auto_pad == "VALID":
        if auto_pad == "NOTSET":
            # Pads [C1]
            pads_value = st.integers(min_value=inputs_attributes['pads_min'], 
                                     max_value=inputs_attributes['pads_max'])
            # Pads [C2]
            pads = draw(st.lists(
                            pads_value,
        ...
        w_spatial_axis = [draw(st.integers(min_value=inputs_attributes['w_spatial_axis_min'], 
                                           max_value=inputs_attributes['w_spatial_axis_max'])) for i in range(num_spatial_axes_w)]
        
        # Dilations [C1] 
        # Dilations [C2]
        dilations = [draw(st.integers
                            (min_value=inputs_attributes['dilation_min'],
                             max_value=inputs_attributes['dilation_max'])) for _ in range(num_spatial_axes_w)]
    ...
    if auto_pad == "SAME_UPPER" or auto_pad == "SAME_LOWER":
        pads = []
        dilations = []
        spatial_w_dimension_values = st.integers(min_value=inputs_attributes['w_spatial_axis_min'],
                                            max_value=inputs_attributes['w_spatial_axis_max'])
    ...
    # dw0 is the first dimension of W, i.e., the number of output channels
    # W [C5]
    all_valid_dw0 = [i for i in range(inputs_attributes['min_dw0'], inputs_attributes['max_dw0'] + 1) if i % groups == 0]
    dw0 = draw(st.sampled_from(all_valid_dw0))

    # dw1 is the second dimension of W, i.e., the number of input channels divided by the number of groups
    ...
    #---------------------------------------------------
    # B [C1] 
    # db0 is the first dimension of B, i.e., the number of output channels
    db0 = dw0
...
def teardown_module():
    data = {
        ...
        "min_dx0": inputs_attributes['min_dx0'],
        "max_dx0": inputs_attributes['max_dx0'],
        "dx0": generated_data["dx0"],
        "min_dx1": inputs_attributes['min_dx1'],
        "max_dx1": inputs_attributes['max_dx1'],
        "dx1": generated_data["dx1"],
        "x_spatial_axis_min": inputs_attributes['x_spatial_axis_min'],
        "x_spatial_axis_max": inputs_attributes['x_spatial_axis_max'],
        "x_spatial_axis": generated_data["x_spatial_axis"],
        "x": generated_data["x"],
        "min_dw0": inputs_attributes['min_dw0'],
        "max_dw0": inputs_attributes['max_dw0'],
        "dw0": generated_data["dw0"],
        "w_spatial_axis_min": inputs_attributes['w_spatial_axis_min'],
        "w_spatial_axis_max": inputs_attributes['w_spatial_axis_max'],
        "w_spatial_axis": generated_data["w_spatial_axis"],
        "w": generated_data["w"],
        "db0": generated_data["db0"],
        "bias": generated_data["bias"],
        "strides_min": inputs_attributes['strides_min'],
        "strides_max": inputs_attributes['strides_max'],
        "strides": generated_data["strides"],
        "auto_pad": generated_data["auto_pad"],
        "pads_min": inputs_attributes['pads_min'],
        "pads_max": inputs_attributes['pads_max'],
        "pads": generated_data["pads"],
        "dilation_min": inputs_attributes['dilation_min'],
        "dilation_max": inputs_attributes['dilation_max'],
...
def check_constraints(x, w, auto_pad, y, dy2,
    ...
    # auto_pad [C1]
    all_valid_autopad = ["NOTSET", "SAME_UPPER", "SAME_LOWER", "VALID"] 
    assert auto_pad in all_valid_autopad
    # auto_pad [C2]
    if auto_pad != "NOTSET":
