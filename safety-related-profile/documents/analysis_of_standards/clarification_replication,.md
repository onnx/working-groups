...
Our work is guided by industrial needs and regulatory bodies recommandations. In this regards, we are considering the recommandations given by the ARP 6983, and in particular those related to the MLMD.

We have the following set of questions.

##  (§6.4.3.6) ML Model description 

### Q1: Model control and data flows
...
The model is a concrete representation of a graph, which is an abstract mathematical construct. So using the terms "interfaces" and "control flow" seems a bit strange here... 

*What is the actual intent for using these terms?*
...
Hyperparameters are also defined in §(6.4.3.3): "Examples of model building hyperparameters for neural networks include (but are not limited to): number of layers, number of neurons in each layer, and their connections, selection of the activation functions in each layer, algorithm hyperparameters such as learning rate."

*What is the actual intent of providing parameters related to the training phase (e.g., learning rate)? \
Is the intent to support a completely reproducible training phase?  If it is the case, other data should also be provided such as the training dataset (of course), the exact description of the target machine, training framework, etc.*

---

## (§7.1) ML Constituent Architecture Design Process

**//!\ To be discussed before sending this to the WG114. /!\\**
...
*What does "inputs" refer to? Are we refering to the design process inputs (i.e., the engineering items used to perform the design) or to the cctual inputs of the ML consitutent?*
...
*What is meant by "similar enough"?*
...
Later in the same section, it is written that the MLMD  *include** 
...
*A first look, the two definitions do not seem to be identical. The first one express the fact that the semantics must be the same where as the second one express that the "performance, generalization, ..." must be preserved. Is this correct?*
