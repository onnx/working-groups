...
$$\begin{gathered}
    Y[b, c, m, n] = \sum_{i=0}^{dW_1-1} \sum_{j=0}^{dW_2-1} \sum_{z=0}^{dW_3-1} \\ (X_{\text{eff}}[b,i,m \cdot strides[0]+ j , n \cdot strides[1]+ z ] \cdot W_{\text{eff}}[c, i, j, z]) \\ + B_{\text{eff}}[c]
\end{gathered}$$

Where
- $b$ is the batch index, $b \in [0,dY_0-1]$, $dY_0$ is the batch size of output `Y`
- $c$ is the data channel, $c \in [0,dY_1-1]$, $dY_1$ is the number of data channels of output `Y`
...
- $dW_3$ is the sizes of the second spatial axis of kernel `W`
...
$$\begin{gathered}
    Y[b, c, m, n] = \sum_{j=0}^{dW_2-1} \sum_{z=0}^{dW_3-1}\\ (X_{\text{eff}}[b, c, m \cdot strides[0] + j , n \cdot strides[1] + z \cdot ] \cdot W_{\text{eff}}[c, 0, j , z] ) + B_{\text{eff}}[c]
\end{gathered}$$
...
###### Constraints
- `C<1>` &lt;Consistency between the number of channels of `X` and `W`&gt;
...
- `C<2>` &lt;Consistency between the shape of `X` and the length of `pads`&gt;
...
- `C<3>` &lt;Consistency between the shape of tensors `X`, `W`, `Y` and  attributes `pads`, `dilations` and `strides`&gt;  
    - Statement: [See constraint (C3) of X](#shape_consist)
...
- `C<2>` &lt;Relation between `dilations` and `W`&gt; 
...
###### Constraints
- `C<1>` &lt;Support for standard and depthwise convolutions&gt;
...
- `C<2>` &lt;Consistency between `W` and `kernel_shape`&gt;
...
###### Constraints.
- `C<1>` &lt;Consistency between the shape of tensors `X`, `W`, `Y`, attributes `pads` and `strides`&gt;
    - Statement: [see constraint (C3) of X](#shape_consist)
